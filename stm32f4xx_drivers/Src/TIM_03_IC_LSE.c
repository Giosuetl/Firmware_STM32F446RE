...
void TIM_setup(void)
{
	...
	htim2.TIM_TimeBase.TIM_Period = 0xffffffff;//periodo al maximo
...
{	//se configurara una freq de reloj a 168mhz por que originalmente a 16 la recuencia de interrupcion era mas rapida
	//que la frecuencia de procesamiento por lo que no se lograba eliminar la flag de interrupcion
	...
	RCC_APB1CLKConfig(RCC_APB_Prescaler_4); // 42MHz// si se tiene de entrda 168 como el maximo de apb1 es 45 como minimo se debe enviar un preescaler de 4
	RCC_APB2CLKConfig(RCC_APB_Prescaler_2); // 84MHz//si se tiene de entrada 168 como el maximo de apb2 es de 90 se establece una freq de 84
...
int main(void)
{
	...
	TIM_ITConfig(htim2.pTIMx, TIM_IT_CC2I, ENABLE);//habilitar la interrupcion capture/capture canal 2 modo entrada
	TIM_Cmd(htim2.pTIMx, ENABLE);

	NVIC_IRQ_Enable(IRQ_TIM2);//habilitar IRQ
	...
	if(event == TIM_EVENT_CC2I){
		...
		case waiting_for_cap2_st:
			...
			if(capture2 > capture1)
				...
				capture_diff = 0xffffffff - capture1 + capture2 + 1; //conteo anti desborde
			freq_measurement_st = processing_st;
			break;
		case processing_st:
			break;//en caso de que salga una interrupcion mientras esta en el procesamiento, esta sera ignorada
		default:
			freq_measurement_st = waiting_for_cap1_st;
			break;
		}
