...
uint8_t TIM_GetITStatus(TIM_RegDef_t *pTIMx, uint16_t TIM_IT)
{
	...
	if ((IT_status != (uint16_t)RESET) && (IT_enable != (uint16_t)RESET))//verificar que el flag este activado y la interrupcion habilitada
		return SET;
...
{
	pTIMx->SR = (uint16_t)~TIM_IT;//limpia el bit del flag en el status register [BIF] page 501/1328
}
...
void TIM_TimeBaseInit(TIM_handle_t *pTIMHandle)
{
	...
	pTIMHandle->pTIMx->ARR = pTIMHandle->TIM_TimeBase.TIM_Period;//el periodo va a el auto reload register
	pTIMHandle->pTIMx->PSC = pTIMHandle->TIM_TimeBase.TIM_Prescaler;//prescalaer para la funcion clk/(pres+1)
	...
	pTIMHandle->pTIMx->EGR = 1;//reinicializar el contador y los registros page 567/1328
}
...
void TIM_ICInit(TIM_handle_t *pTIMHandle)
{
	...
	if(pTIMHandle->TIM_InputCapture.TIM_Channel == TIM_CH1){
		...
		temp_CCMRx = pTIMHandle->pTIMx->CCMR1;//capture/compare mode register
		temp_CCER = pTIMHandle->pTIMx->CCER;//capture/compare enable register

		// Configurar entrada en registro CCMR1//en el registro CCMR1 se puede configurar el canal 1 y canal 2 pag 568/1328
		temp_CCMRx &= ~(0x03 << TIM_CCMR1_CC1S);
		temp_CCMRx |= (pTIMHandle->TIM_InputCapture.TIM_ICSelection << TIM_CCMR1_CC1S);

		// Configurar filtro en registro CCMR1// -> define la frecuencia utilizada para muestrear la entrada TI1 y la longitud del filtro digital aplicado a TI1.
		temp_CCMRx &= ~(0x0F << TIM_CCMR1_IC1F);//El filtro digital está compuesto por un contador de eventos en el que se necesitan N eventos consecutivos para validar una transición en la salida
...
void TIM_OCInit(TIM_handle_t *pTIMHandle)
{
	...
	if(pTIMHandle->TIM_OutputCompare.TIM_Channel == TIM_CH1){
		// Reset del bit CC1E para deshabilitar el canal
		pTIMHandle->pTIMx->CCER &= ~(1 << TIM_CCER_CC1E);//page 572/1328

		temp_CCMRx = pTIMHandle->pTIMx->CCMR1;//page 568/1328
		temp_CCER = pTIMHandle->pTIMx->CCER;
...
{
	if(status == ENABLE){//page 499/1328
		pTIMx->DIER |= IT_type;// realizado con mascara de bits
	}else{
		pTIMx->DIER &= ~IT_type;
	}
}
...
{
	if(TIM_GetITStatus(pTIMHandle->pTIMx, TIM_IT_UI)){//si se habilida el flag y la interrupcion
		TIM_ClearITPendingBit(pTIMHandle->pTIMx, TIM_IT_UI);//limpia flag
