...
void RCC_MCO1Config(RCC_MCO1_Source_t MCO1_Source, RCC_MCO1_Prescaler_t MCO1_Prescaler)
{
	...
	temp_RCC_CFGR &= ~(0x03 << RCC_CFGR_MCO1);// se limpian dos bits 21 y 22
	// Seteamos los bit de source
	temp_RCC_CFGR |= (MCO1_Source << RCC_CFGR_MCO1);

	// Limpiar los bits de prescaler
	temp_RCC_CFGR &= ~(0x07 << RCC_CFGR_MCO1PRE); // se limpian 3 bits 0x07 page 131/1328
...
void RCC_AHBCLKConfig(RCC_AHB_Prescaler_t AHB_Prescaler)
{
	...
	temp_RCC_CFGR &= ~(0x0F << RCC_CFGR_HPRE); //0x0F = 1111 (cuatro bits) page 133/1328
...
uint32_t RCC_GetSYSCLK(void)
{
	...
	rcc_cfgr_sws = (RCC->CFGR >> RCC_CFGR_SWS) & 0x03; //131/1328 lee fuente de reloj del sys clock shift de dos posiciones 3 en hexadecimal = dos bit
	...
	if(rcc_cfgr_sws == 0){
		 ...
		 *  SYSCLK = PLL_VCO / PLLP
		 ***************************************************/
		PLL_src = (RCC->PLLCFGR >> RCC_PLLCFGR_PLLSRC) & 0x01;//verificar el estado del bit 22 page 129/1328 shift de 22 y mascara para el primer bit
		PLL_M = RCC->PLLCFGR & 0x3F;// para los 6 primero bits mascara de 3F 129/1328
		PLL_N = (RCC->PLLCFGR >> RCC_PLLCFGR_PLLN) & 0x1FF;// shtf de 6 posiciones y mascara de 9 bits
		...
		PLL_P = (((RCC->PLLCFGR >> RCC_PLLCFGR_PLLP) & 0x03) + 1) * 2;//0x03= dos bits , conversion (n+1)*2
		freq = PLL_VCO / PLL_P;
	}if(rcc_cfgr_sws == 3){
		/********************** PLL R **********************
		 *  PLL_VCO = (PLL_src / PLLM) * PLLN
		...
		PLL_R = (RCC->PLLCFGR >> RCC_PLLCFGR_PLLR) & 0x07;////shift de 28 y mascara con 7
		freq = PLL_VCO / PLL_R;
	}
